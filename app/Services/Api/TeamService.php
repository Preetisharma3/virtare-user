    ...
    public function team($request, $type, $id)
    {
        if ($type == 'staff') {
            ...
            if (!$id) {
                ...
                return fractal()->collection($data)->transformWith(new PatientPhysicianTransformer(true))->paginateWith(new IlluminatePaginatorAdapter($data))->toArray();
            } else {
                $data = PatientPhysician::where([['patientId', auth()->user()->patient->id], ['id', $id]])->first();
                if (!empty($data)) {
                    return fractal()->item($data)->transformWith(new PatientPhysicianTransformer(true))->toArray();
                } else {
                    return response()->json(['message' => trans('messages.not_found')], 404);
                }
            }
            ...
            if (!$id) {
                $data = PatientFamilyMember::where([['patientId',auth()->user()->patient->id]])->paginate(5);
