    ...
    public function patientConditionCreate($request, $id)
    {
        ...
        try {
            if (!$id) {
                ...
                $conditionUpdate = ['deletedBy' => 1, 'isDelete' => 1, 'isActive' => 0];
                PatientCondition::where('patientId', $patientId)->update($conditionUpdate);
                ...
                foreach ($conditions as $condition) {
                    ...
                    $result = DB::select(
                        "CALL getPatientCondition('" . $patientId . "')"
                    );
        ...
        try {
            if (!$id) {
                ...
                if (!$referalsId) {
                    ...
                    $getPatient = PatientReferal::where('id', $patient->id)->first();
                    $result = DB::select(
                        "CALL getPatientReferal('" . $getPatient->udid . "','" . $patientId . "')"
                    );
                    ...
                    $input = [
                     ...
                     PatientReferal::where('udid', $referalId)->update($input);
                    $result = DB::select(
                        "CALL getPatientReferal('" . $referalsId . "','" . $patientId . "')"
                    );
                    $userdata = fractal()->collection($result)->transformWith(new PatientReferalTransformer())->toArray();
                    $message = ['message' => 'update successfully'];
                }
    ...
    public function patientReferalsList($request, $id, $referalsId)
    {
        try {
            if (!$id) {
                ...
                if (!$referalsId) {
                    $result = DB::select(
                        "CALL getPatientReferal('" . '' . "','" . $patientId . "')"
                    );
                } else {
                    $result = DB::select(
                        "CALL getPatientReferal('" . $referalsId . "','" . $patientId . "')"
        ...
        try {
            if (!$id) {
                ...
                if (!$physicianId) {
                    ...
                    $getPatient = PatientPhysician::where('id', $patient->id)->first();
                    $result = DB::select(
                        "CALL getPatientPhysician('" . $getPatient->udid . "','" . $patientId . "')"
                    );
                    ...
                    $result = DB::select(
                        "CALL getPatientPhysician('" . $physicianId . "','" . $patientId . "')"
                    );
                    $userdata = fractal()->collection($result)->transformWith(new PatientPhysicianTransformer())->toArray();
                    $message = ['message' => 'update successfully'];
                }
    ...
    public function patientPhysicianList($request, $id, $physicianId)
    {
        try {
            if (!$id) {
                ...
                if (!$physicianId) {
                    $result = DB::select(
                        "CALL getPatientPhysician('" . '' . "','" . $patientId . "')"
                    );
                } else {
                    $result = DB::select(
                        "CALL getPatientPhysician('" . $physicianId . "','" . $patientId . "')"
        ...
        try {
            if (!$id) {
                ...
                if (!$programId) {
                    ...
                    $result = DB::select(
                        "CALL getPatientProgram('" . $programId . "','" . $patientId . "')"
                    );
                }
            } else {
                $patient = Patient::where('udid', $id)->first();
        ...
        try {
            if (!$id) {
                ...
                if (!$inventoryId) {
                    ...
                    $inventory = PatientInventory::create($input);
                    $inventoryId = PatientInventory::where('id', $inventory->id)->first();

                    $result = DB::select(
                        "CALL getPatientInventory('" . $inventoryId->id . "','" . $patientId . "')"
                    );
                    $userdata = fractal()->collection($result)->transformWith(new PatientInventoryTransformer())->toArray();
                    $message = ['message' => 'created successfully'];
                } else {
                    $input = [
                        'isActive' => $request->input('status'),  'updatedBy' => Auth::id(),
                    ];
                    PatientInventory::where('udid', $inventoryId)->update($input);
                    $patient = PatientInventory::where('udid', $inventoryId)->first();
                    $result = DB::select(
                        "CALL getPatientInventory('" . $patient->id . "','" . $patientId . "')"
                    );
                    $userdata = fractal()->collection($result)->transformWith(new PatientInventoryTransformer())->toArray();
                    $message = ['message' => 'updated successfully'];
                }
    ...
    public function patientInventoryList($request, $id, $inventoryId)
    {
        try {
            if (!$id) {
                ...
                if ($inventoryId) {
                    $result = DB::select(
                        "CALL getPatientInventory('" . $inventoryId . "','" . $patientId . "')"
                    );
                } else {
                    $result = DB::select(
                        "CALL getPatientInventory('" . '' . "','" . $patientId . "')"
                    );
                }
            } else {
                $patient = Patient::where('udid', $id)->first();
        ...
        try {
            if (!$id) {
                ...
                if (!$medicalHistoryId) {
                    ...
                    $result = DB::select(
                        "CALL getPatientMedicalHistory('" . $patient->udid . "','" . $patientId . "')"
                    );
                    $userdata = fractal()->collection($result)->transformWith(new PatientMedicalTransformer())->toArray();
                    $message = ['message' => 'created successfully'];
                } else {
                    $input = [
                    ...
                    $result = DB::select(
                        "CALL getPatientMedicalHistory('" . $medicalHistoryId . "','" . $patientId . "')"
                    );
                    $userdata = fractal()->collection($result)->transformWith(new PatientMedicalTransformer())->toArray();
                    $message = ['message' => 'updated successfully'];
                }
                ...
                if (!$medicalHistoryId) {
                    ...
                    $patient = PatientMedicalHistory::create($input);
                    $patientData = PatientMedicalHistory::where('id',$patient->id)->first();
                    
                    $result = DB::select(
                        "CALL getPatientMedicalHistory('" . $patientData->udid . "','" . $patientId . "')"
                    );
                    $userdata = fractal()->collection($result)->transformWith(new PatientMedicalTransformer())->toArray();
                    $message = ['message' => 'created successfully'];
                } else {
        ...
        try {
            if (!$id) {
                ...
                if ($medicalHistoryId) {
                    ...
                    $result = DB::select(
                        "CALL getPatientMedicalHistory('" . '' . "','" . $patientId . "')"
                    );
                }
            } else {
                $patient = Patient::where('udid', $id)->first();
            ...
            return fractal()->collection($result)->transformWith(new PatientMedicalTransformer())->toArray();
    ...
    public function patientMedicalRoutineList($request, $id, $medicalRoutineId)
    {
        try {
            if (!$id) {
                ...
                if ($medicalRoutineId) {
                    $result = DB::select(
                        "CALL getPatientMedicalRoutine('" . $medicalRoutineId . "','" . $patientId . "')"
                    );
                } else {
                    $result = DB::select(
                        "CALL getPatientMedicalRoutine('" . '' . "','" . $patientId . "')"
                    );
                }
            } else {
                $patient = Patient::where('udid', $id)->first();
            ...
            return fractal()->collection($result)->transformWith(new PatientMedicalRoutineTransformer())->toArray();
    ...
    public function patientInsuranceList($request, $id, $insuranceId)
    {
        try {
            if (!$id) {
                ...
                if ($insuranceId) {
                    $result = DB::select(
                        "CALL getPatientInsurance('" . $insuranceId . "','" . $patientId . "')"
                    );
                } else {
                    
                    $result = DB::select(
                        "CALL getPatientInsurance('" . '' . "','" . $patientId . "')"
                    );
                }
            } else {
                $patient = Patient::where('udid', $id)->first();
            ...
            return fractal()->collection($result)->transformWith(new PatientInsuranceTransformer())->toArray();
        } catch (Exception $e) {
            return response()->json(['message' => $e->getMessage()],  500);
        }
    }
