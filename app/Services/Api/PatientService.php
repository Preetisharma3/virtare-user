...
class PatientService
{
    ...
    public function patientCreate($request, $id)
    {
        ...
        try {
            if (!$id) {
                ...
                $message = ['message' => trans('messages.user_created')];
            } else {
                ...
                $userdata = fractal()->item($getPatient)->transformWith(new PatientTransformer())->toArray();
                $message = ['message' => trans('messages.user_updated')];
            }
            DB::commit();
        ...
        try {
            if (!$referalsId) {
                ...
                $input = [
                    ...
                    'patientId' => $patientId->id, 'fax' => $request->input('fax'), 'createdBy' => Auth::id(), 'phoneNumber' => $request->input('phoneNumber'), 'udid' => $udid
                ];
    ...
    public function patientPhysicianCreate($request, $id, $physicianId)
    {
        ...
        try {
            if (!$physicianId) {
                ...
                $patientId = Patient::where('udid', $id)->first();
                $input = [
                    'sameAsReferal' => $request->input('sameAsAbove'), 'patientId' => $patientId->id, 'fax' => $request->input('fax'),
    ...
    public function patientInventoryList($request, $id, $inventoryId)
    {
        try {
            ...
            if ($data) {
                if ($request->latest) {
                    $getPatient = PatientInventory::where('patientId', $data->id)->with('patient', 'inventory', 'deviceTypes')->latest()->first();
    ...
    public function patientInventoryDelete($request, $id, $inventoryId)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['message' => trans('messages.updated_succesfully')]);
        } catch (Exception $e) {
            DB::rollback();
            return response()->json(['message' => $e->getMessage()],  500);
        }
    ...
    public function patientVitalDelete($request, $id, $vitalId)
    {
        ...
        try {
            $data = ['deletedBy' => Auth::id(), 'isDelete' => 1, 'isActive' => 0];
            PatientVital::where('udid', $vitalId)->update($data);
            PatientVital::where('udid', $vitalId)->delete();
            DB::commit();
            return response()->json(['message' => trans('messages.deleted_succesfully')]);
        } catch (Exception $e) {
            DB::rollback();
    ...
    public function patientMedicalHistoryCreate($request, $id, $medicalHistoryId)
    {
        ...
        try {
            if (!$medicalHistoryId) {
                $patientId = Patient::where('udid', $id)->first();
                $input = [
                    'history' => $request->input('history'), 'patientId' => $patientId->id,  'createdBy' => Auth::id(), 'udid' => Str::uuid()->toString()
                ];
        ...
        try {
            if (!$medicalRoutineId) {
                ...
                $input = [
                    ...
                    'startDate' => date("Y-m-d", $request->input('startDate')), 'endDate' => date("Y-m-d", $request->input('endDate')), 'patientId' => $patientId->id, 'udid' => $udid
                ];
    ...
    public function patientMedicalRoutineList($request, $id, $medicalRoutineId)
    {
        try {
            if ($medicalRoutineId) {
                ...
                $patientId = Patient::where('udid', $id)->first();
                $getPatient = PatientMedicalRoutine::where('patientId', $patientId->id)->with('patient')->orderBy('createdAt', 'DESC')->get();
    ...
    public function patientTimeLogAdd($request, $entityType, $id, $timelogId)
    {
        ...
        try {
            if (!$timelogId) {
                ...
                $data = response()->json(['message' => trans('messages.created_succesfully')]);
            } else {
                $dateConvert = Helper::date($request->input('date'));
                $timeConvert = Helper::time($request->input('timeAmount'));
                $timeLog = array();
                ...
                $data = response()->json(['message' => trans('messages.updated_succesfully')]);
            }
            DB::commit();
            return $data;
        } catch (Exception $e) {
